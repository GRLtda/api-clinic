...
const DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/1432810373244915732/OapA83WGKuWf1rlxbtQGFPkwD_H4K9mIxtO8BaIKrO1ZVyT5u5UNyKLVi_U0u0Ce41D1';
...
const sendDiscordNotification = (color, title, fields, footer) => {
    ...
    axios.post(DISCORD_WEBHOOK_URL, payload)
        .catch(err => captureException(err, { tags: { severity: 'discord_webhook_failure', context: 'sendDiscordNotification' } }));
};
...
const trySendMessageAndLog = async ({
    ...
    try {
        ...
        sendDiscordNotification(notificationColor, notificationTitle, notificationFields, `Status: ${LOG_STATUS.DELIVERED}`);
        ...
        sendDiscordNotification(notificationColor, notificationTitle, notificationFields, `Status: ${LOG_STATUS.ERROR_SYSTEM}`);
    }
};
...
const checkAndSendAppointmentReminders = async (type, daysOffset) => {
    ...
    if (daysOffset > 0) {
        ...
        targetStartUtc = nowUtc.plus({ minutes: 1 });
        targetEndUtc = nowUtc.plus({ minutes: 2 });
    } else {
        return;
    }
    ...
    const settingProcessingPromises = activeSettings.map(async (setting) => {
        if (!setting.template?.content || !setting.clinic?._id || !setting.clinic.owner?.name) return;
        ...
        const sendTasks = appointments.map(appointment => {
          if (!appointment.patient?._id || !appointment.patient.phone) return Promise.resolve();
...
const runTask = async (taskName, ...args) => {
    ...
    try {
        ...
        captureException(error, { tags: { severity: 'worker_task_failure', task: taskName } });
