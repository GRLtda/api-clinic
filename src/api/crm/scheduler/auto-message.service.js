...
const runTaskInWorker = (taskName) => {
  ...
  sendToDiscord(
    `Iniciando worker para a tarefa: **${taskName}**`,
    "info",
    taskName
  );
  ...
  worker.on("message", (msg) => {
    if (msg.status === "error") {
      console.error(
        `[SCHEDULER] Worker Error for task ${msg.taskName}:`,
        msg.error
      );
      captureException(new Error(`Worker failed: ${msg.error}`), {
        tags: { severity: "worker_general_failure", task: msg.taskName },
      });

      sendToDiscord(
        `Worker reportou um erro: \`\`\`${msg.error}\`\`\``,
        "error",
        msg.taskName
      );
      ...
      console.log(
        `[SCHEDULER] Worker task ${msg.taskName} concluída com sucesso.`
      );
  ...
  worker.on("error", (err) => {
    console.error(
      `[SCHEDULER] Worker Thread fatal error for task ${taskName}:`,
      err
    );
    captureException(err, {
      tags: { severity: "worker_fatal_failure", task: taskName },
    });

    sendToDiscord(
      `Worker teve um erro FATAL: \`\`\`${err.message}\`\`\``,
      "error",
      taskName
    );
  });
  worker.on("exit", (code) => {
    if (code !== 0) {
      console.error(
        `[SCHEDULER] Worker Thread for task ${taskName} stopped with exit code ${code}.`
      );

      sendToDiscord(
        `Worker parou inesperadamente com código de saída: **${code}**`,
        "warn",
        taskName
      );
    }
...
exports.startAutoMessageScheduler = () => {
  ...
  // CRON JOB 1: A CADA MINUTO (Verifica lembretes de 3 minutos com janela de tolerância)
  // Alterado de "*/3 * * * *" para "* * * * *"
  cron.schedule("* * * * *", () => {
    // Alterado taskName para APPOINTMENT_3_MINS_BEFORE
