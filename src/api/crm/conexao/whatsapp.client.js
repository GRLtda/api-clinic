...
/**
 * Função para disparar retry de mensagens pendentes quando cliente conecta
 * @param {string} clinicId
 */
const triggerRetryOnConnection = async (clinicId) => {
  try {
    const {
      retryPendingMessages,
    } = require("../scheduler/auto-message.service");
    console.log(
      `[${clinicId}] Disparando retry de mensagens pendentes após conexão...`
    );
    await retryPendingMessages(clinicId);
  } catch (error) {
    console.error(
      `[${clinicId}] Erro ao executar retry após conexão:`,
      error.message
    );
  }
};
...
const initializeClient = async (clinicId) => {
  ...
  client.on("ready", () => {
    ...
    // Dispara retry de mensagens pendentes quando conecta
    setTimeout(() => {
      triggerRetryOnConnection(id);
    }, 2000); // Aguarda 2 segundos para garantir que a conexão está estável
  });
  ...
  client.on("disconnected", (reason) => {
    clientStatus.set(id, "disconnected");
    console.log(
      `[${id}] Client was disconnected! Reason: ${reason}. Status: ${getClientStatus(
        id
      )}`
    );
  });
...
const destroyClient = async (clinicId) => {
  ...
  if (client) {
    try {
      ...
      console.error(
        `[${id}] Erro ao destruir/deslogar cliente:`,
        error.message
      );
    }
    clients.delete(id);
