...
const generateSamplePatient = (clinicId) => {
  ...
  const gender = fakerPT_BR.helpers.arrayElement([
    "Masculino",
    "Feminino",
    "Outro",
  ]);
  ...
  const birthDate = fakerPT_BR.date.past({
    years: 90,
    refDate: new Date(Date.now() - 18 * 365 * 24 * 60 * 60 * 1000),
  });
  const phone = fakerPT_BR.string.numeric(11); // Gera 11 dígitos numéricos
...
const runAutomation = async () => {
  ...
  try {
    ...
    const numberOfPatients = parseInt(
      readline.question("Quantos pacientes deseja criar? ")
    );
    ...
    const result = await Patient.insertMany(patientsToCreate, {
      ordered: false,
    });
    ...
    if (error.code === 11000 || error.writeErrors) {
      const duplicateErrors =
        error.writeErrors?.filter((e) => e.err.code === 11000).length || 1;
      console.error(
      ...
      if (successfulInserts > 0) {
        console.log(
          `${successfulInserts} pacientes foram criados com sucesso.`
        );
      }
    } else {
